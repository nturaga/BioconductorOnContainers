...
monitor the size of your containers as it may grow very quickly. 
Read more about [Container size on disk][].
...
[bioconductor docker page][] has information and instructions on how to 
use them. The [Dockerfile(s)][] for each of these containers is located in 
Github. These are provided directly by the Bioconductor team for each 
...
Bioconda provides [**BioContainers**][].  Bioconda
provides a container for each individual package in Bioconductor under 
the "BioContainer" umbrella in [Quay.io][].
...
Bioconductor packages. Bioconda does not build or distribute 'devel' version
of Bioconductor.
...
But please note, that the actual Bioconductor packages themselves don't come on this image. It is up to
the user to install the Bioconductor packages through `BiocManager::install`.
...
of the way these images are built and provisioned as docker images. The list 
is given in the `docs/issues.md` files on the Github page for these images. 
(We try to keep that document as updated as possible, but if you find a package 
installs and is listed there, send us a Pull Request.)
...
For most beginners a good place to start is the `bioconductor/release_core2` docker
container which includes all the "core" bioconductor packages.
...
This location `/usr/local/lib/R/host-site-library` has special meaning, and the configuration within the
...
instantly the next time R is started from the container. 
Another big advantage is, it prevents your docker image
from growing in size due to package installation and data. This allows
the users to distribute smaller docker images containing only what is needed.
...
The advanced section is for anyone who has some familiarity with how Docker works, 
and wants to run things on the "Cloud". This will be a demonstration section, 
since we are not providing "credits" to use a certain cloud (Google or AWS).

This section aims to give some direction to users who are looking to use Bioconductor
on the cloud. We also discuss some future work, which look very promising.
...
cheaper and faster for example 1000 Genomes data which is available 
on the google cloud: gs://genomics-public-data/1000-genomes.
...
1. Launch a Bioconductor docker image on the Google Cloud using the
   ...
   project. When you are ready to restart it is easy enough to restart.
...
You can mount storage buckets on your VM to your container using GCSFUSE. 
For this you need to choose an image with access to the google cloud and gcsfuse. 
To install gcsfuse, and
...
We also provide a very helpful package (which is part of another project)
...
Singularity is a container engine just like Docker. Read more about on the [Singularity
webpage][].
...
Hub][]. There are three containers provided right now which reflect,
...
`-B` is how you mount a volume on the singularity image `<local path>:<container path>`. 
This command shows my R library being mounted from my head node on the singularity container.
...
Use BiocParallel to launch and run jobs on Singularity containers. The following example has been run on `SLURM`. Thanks to Levi Waldron and Ludwig Geistlinger.
...
For a SLURM cluster, `slurm-singularity.tmpl` is the template file with contents,
...
singularity shell <path-to-image>/<bioconductor-image-w-BiocParallel>-latest.simg R -e 'batchtools::doJobCollection("<%= uri %>")'
```

Using `BiocParallel` and `batchtools`, it is then possible to run the following on
your head node in R
...
## Run bplapply
res <- bplapply(1:20, test_cluster, BPPARAM = param)
...
There is quite a bit of work in progress making Bioconductor meet data
analysis requirements on the cloud for a range of work.
