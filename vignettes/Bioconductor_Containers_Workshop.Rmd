...
Nitesh Turaga* <nitesh.turaga@roswellpark.org> (Github:
...
containers) plus one container’s (virtual size- size). Be careful and
monitor the size as it may grow very quickly.
...
To be able to use the containers on HPC cluster facilities, a reason
to use Singularity containers because Singularity is works on a BSD
license (open source!!), and works very well with Docker images which
are already available.  To read more about it visit the
[Singularity webpage](https://singularity.lbl.gov/about).

Bioconductor provides Singularity containers on the
[Singularity Hub](https://www.singularity-hub.org/collections/3154). There are
three containers provided right now which reflect,
...
system admin for new software installtions.
