...
you can do on the Google Cloud you can do with AWS as well
...
1. Google cloud billing account
	...
	large compute instances. (https://cloud.google.com/free/)
...
1. Launch a Bioconductor docker image on the Google Cloud using the
   ...
   computer accessible remotely) need to accessible via the HTTP
   protocol at a specific port (*8787* for RStudio). For this we need
   to create a firewall rule which allows HTTP access at that port
   8787.
...
You can mount volumes on your VM to your container using GCSFUSE.
...
instances, as provide as a demonstration ONLY google storage buckets with
all the software packages which are pre-compiled (1600+).
...
NOTE: 'Requester Pays' (aka: You) for the access to these buckets
(https://cloud.google.com/storage/docs/requester-pays). So by
accessing the data on these packages to be downloaded on to your Cloud
instance your billing project gets charged.
...
To copy packages over to the instance, using the command line:
	...
	gsutil \
		...
		-m \
		cp -r gs://biocondcutor-full-devel/<package_name> <local_directory>

<TODO> : Should there be a quickly designed R package to do this for
the users? GoogleCloudBiocInstaller

### HPC-Singularity containers

Singularity is a container engine just like Docker.

To be able to use the containers on HPC cluster facilities, a reason to 
use Singularity containers because Singularity is works on a BSD license (open source!!), 
and works very well with Docker images which are already available. 
To read more about it visit the [Singularity webpage](https://singularity.lbl.gov/about).

Bioconductor provides Singularity containers on the 
[Singularity Hub](https://www.singularity-hub.org/collections/3154). 
There are three containers provided right now  which reflect,
...
Singularity comes as a module which can be installed on a cluster facility. 
Once the module is available, it is easy for the user to run software in a
containerized fashion, without depending on the system admin for new software 
installtions.
...
1. BiocParallel::RedisParam
	...
	It is based on the *redis* data-structure (https://redis.io/).

	Github link: https://github.com/mtmorgan/RedisParam/tree/master
    ...
    backend allows computation to be distributed across instances or
...
2. Kubernetes
	...
	<TODO:> moved to Bioconductor
	Link: https://github.com/Bioconductor/k8s-redis-bioc-example/tree/master/k8s
	
3. Helm charts

	[helm.sh](www.helm.sh) is a package manager for Kubernetes. Once
    ...
    distributable tarball (or package).

	<Short Demo of Helm/K8S on google cloud>

	Link: https://github.com/mtmorgan/k8s-redis-bioc-example/tree/master/helm-chart
