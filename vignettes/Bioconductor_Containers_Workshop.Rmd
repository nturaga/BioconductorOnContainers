...
Running your R session on the cloud with RStudio makes you
'machine-independent'. It also allows you to access data hosted on the
cloud cheaper/faster to access.
...
(https://cloud.google.com/docs/compare/aws/). There are scripts
included in this package regarding deployment of infrastructure on the
AWS cloud, but the demo will focus on the google cloud.

#### Requirements

1. Google cloud billing account

	Pro-tip: Create a test google account (or try cloud.google.com for
...
the other by natively using the "Compute Engine" service and launching
a container with a specific image stored either on the "Google
Container Registry" (google's Dockerhub) or Dockerhub itself.
...
For faster installation of packages on Bioconductor cloud
installtions, we provide google buckets with all the software packages
which are pre-compiled.
...
To copy packages over to the instance,
